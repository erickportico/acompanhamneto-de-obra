        ...
        (function () {
          ...
          function badge(txt, cor) {
            /* patch109: a tarja da nuvem nao aparece mais na tela */
            try {
              var p109velho = document.getElementById('p104Status');
              if (p109velho && p109velho.parentNode) {
                p109velho.parentNode.removeChild(p109velho);
              }
            } catch (p109e) {}
            return;
          }
        
          function p109BadgeAntigo(txt, cor) {
            ...
            sincronizarChaves(function (ok, erro, recarrega, mudou) {
              ...
              function fim() {
                ...
                if (precisaRecarregar && houveMudanca) {
                  /* PATCH153b: so recarregar se tem sessao valida ou pedido manual */
                  var temSessao = false;
                  try {
                    var _s = JSON.parse(sessionStorage.getItem("painel_seg_sessao_v1") || "null");
                    if (!_s) { _s = JSON.parse(localStorage.getItem("painel_seg_sessao_v1") || "null"); }
                    temSessao = !!(_s && _s.usuario);
                  } catch (eS) { temSessao = false; }
                  if (!temSessao && !pedidoManual) {
                    badge('sem sessao — nao recarregar', '#7f1d1d');
                  } else {
                  var ja = false;
                  ...
                  if (!ja || pedidoManual) {
                    ...
                    /* PATCH154_DISABLED_reload */ void(0); /* era: location.reload() */
                  }
                  }
                }
              }
          ...
          try {
            ...
            localStorage.setItem = function (k, v) {
              setOriginal(k, v);
              /* PATCH154_DISABLED_agendar */ // if (CHAVES.indexOf(String(k)) >= 0) { agendar(1800); }
            };
          ...
          /* PATCH154_DISABLED_sync */ // setInterval(function () { sincronizar(false); }, 60000);
